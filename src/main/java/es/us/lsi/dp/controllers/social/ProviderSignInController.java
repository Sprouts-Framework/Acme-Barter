...
public class ProviderSignInController implements InitializingBean {
	...
	@Autowired
	private BaseActorService actorService;
	...
	/**
	 * Configure the list of sign in interceptors that should receive callbacks
	 * during the sign in process. Convenient when an instance of this class is
	 ...
	 * 
	 * @param providerId
	 *            the provider ID to authorize against
	 * @param request
	 *            the request
	 * @return a RedirectView to the provider's authorization page or to the
	 *         application's signin page if there is an error
	 */
	...
	public RedirectView oauth1Callback(@PathVariable String providerId, NativeWebRequest request) {
		try {
			...
			return redirect(URIBuilder.fromUri(signInUrl + "providerError" + signInUrlAction).build().toString());
		}
	}
	 ...
	 * @param request
	 *            the web request
	 * @return A RedirectView to the target page or the signInUrl if an error
	 *         occurs
	 */
	 ...
	 * authorization, typically because the user denied authorization. Simply
	 * carries the error parameters through to the sign-in page.
	 * 
	 * @param providerId
	 *            The Provider ID
	 * @param error
	 *            An error parameter sent on the redirect from the provider
	 * @param errorDescription
	 *            An optional error description sent from the provider
	 * @param errorUri
	 *            An optional error URI sent from the provider
	 * @param request
	 *            The web request
	...
	private RedirectView handleSignIn(Connection<?> connection, ConnectionFactory<?> connectionFactory, NativeWebRequest request) {
		...
		if (principal != null) {
			Collection<SocialAccount> principalSocialAccounts = principal.getUserAccount().getSocialAccounts();
			if (socialAccount != null && principalSocialAccounts.contains(socialAccount)) {
				socialAccountService.updateCustomer(principal, profile, connection);
			} else {
				socialAccountService.addSocialAccount(principal, profile, connection);
			}
			...
			Authentication auth = new UsernamePasswordAuthenticationToken(userAccount, userDetails.getPassword(), userAccount.getAuthorities());
			SecurityContextHolder.getContext().setAuthentication(auth);
			request.getNativeRequest(HttpServletRequest.class).setAttribute(HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY,
