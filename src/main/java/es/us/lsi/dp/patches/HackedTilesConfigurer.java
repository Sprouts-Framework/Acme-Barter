 ...
 * information about Tiles, which basically is a templating mechanism for web
 * applications using JSPs and other template engines.
 * 
 * <p>
 * The TilesConfigurer simply configures a TilesContainer using a set of files
 * containing definitions, to be accessed by {@link TilesView} instances. This
 * is a Spring-based alternative (for usage in Spring configuration) to the
 * Tiles-provided {@code ServletContextListener} (e.g.
 * {@link org.apache.tiles.extras.complete.CompleteAutoloadTilesListener} for
 * usage in {@code web.xml}.
 * 
 * <p>
 ...
 * definitions. If the list is not specified, the default is
 * {@code "/WEB-INF/tiles.xml"}.
 * 
 * @author mick semb wever
 * @author Rossen Stoyanchev
 * @since 3.2
 */
public class HackedTilesConfigurer implements ServletContextAware, InitializingBean, DisposableBean {

	private static final boolean tilesElPresent = // requires JSP 2.1 as well as
													// Tiles EL module
	ClassUtils.isPresent("javax.servlet.jsp.JspApplicationContext", HackedTilesConfigurer.class.getClassLoader())
			&& ClassUtils.isPresent("org.apache.tiles.el.ELAttributeEvaluator", HackedTilesConfigurer.class.getClassLoader());
	 ...
	 */
	public void setTilesInitializer(final TilesInitializer tilesInitializer) {
	 ...
	 * <p>
	 * See
	 * {@link org.apache.tiles.extras.complete.CompleteAutoloadTilesContainerFactory}
	 ...
	 */
	public void setCompleteAutoload(final boolean completeAutoload) {
		if (completeAutoload) {
			try {
				tilesInitializer = new SpringCompleteAutoloadTilesInitializer();
			} catch (final Exception ex) {
				throw new IllegalStateException("tiles-extras 3.x not available", ex);
			}
		} else {
			tilesInitializer = null;
		}
	}
	 ...
	 * Default is "false".
	 */
	public void setCheckRefresh(final boolean checkRefresh) {
	 ...
	 */
	public void setValidateDefinitions(final boolean validateDefinitions) {
	 ...
	 * {@link org.apache.tiles.definition.UnresolvingLocaleDefinitionsFactory},
	 ...
	 */
	public void setDefinitionsFactoryClass(final Class<? extends DefinitionsFactory> definitionsFactoryClass) {
		this.definitionsFactoryClass = definitionsFactoryClass;
	}

	/**
	 * Set the {@link org.apache.tiles.preparer.factory.PreparerFactory}
	 * implementation to use. Default is
	 * {@link org.apache.tiles.preparer.factory.BasicPreparerFactory}, creating
	 ...
	 */
	public void setPreparerFactoryClass(final Class<? extends PreparerFactory> preparerFactoryClass) {
	 ...
	 */
	public void setUseMutableTilesContainer(final boolean useMutableTilesContainer) {
	...
	public void afterPropertiesSet() throws TilesException {
		final ApplicationContext preliminaryContext = new HackedSpringWildcardServletTilesApplicationContext(servletContext);
		if (tilesInitializer == null) {
			tilesInitializer = new SpringTilesInitializer();
		}
		tilesInitializer.initialize(preliminaryContext);
	}
	...
	private class SpringTilesInitializer extends DefaultTilesInitializer {
		...
		@Override
		protected AbstractTilesContainerFactory createContainerFactory(final ApplicationContext context) {
	...
	private class SpringTilesContainerFactory extends BasicTilesContainerFactory {
		...
		@Override
		public TilesContainer createContainer(final ApplicationContext context) {
			final TilesContainer container = super.createContainer(context);
			return (useMutableTilesContainer ? new CachingTilesContainer(container) : container);
		}

		@Override
		protected List<ApplicationResource> getSources(final ApplicationContext applicationContext) {
			if (definitions != null) {
				final List<ApplicationResource> result = new LinkedList<ApplicationResource>();
		...
		@Override
		protected BaseLocaleUrlDefinitionDAO instantiateLocaleDefinitionDao(final ApplicationContext applicationContext, final LocaleResolver resolver) {
			final BaseLocaleUrlDefinitionDAO dao = super.instantiateLocaleDefinitionDao(applicationContext, resolver);
		...
		@Override
		protected DefinitionsReader createDefinitionsReader(final ApplicationContext context) {
			final DigesterDefinitionsReader reader = (DigesterDefinitionsReader) super.createDefinitionsReader(context);
			reader.setValidating(validateDefinitions);
			return reader;
		}

		@Override
		protected DefinitionsFactory createDefinitionsFactory(final ApplicationContext applicationContext, final LocaleResolver resolver) {

			if (definitionsFactoryClass != null) {
				final DefinitionsFactory factory = BeanUtils.instantiate(definitionsFactoryClass);
				...
				final BeanWrapper bw = PropertyAccessorFactory.forBeanPropertyAccess(factory);
		...
		@Override
		protected PreparerFactory createPreparerFactory(final ApplicationContext context) {
		...
		@Override
		protected LocaleResolver createLocaleResolver(final ApplicationContext context) {
			return new HackedSpringLocaleResolver();
		}

		@Override
		protected AttributeEvaluatorFactory createAttributeEvaluatorFactory(final ApplicationContext context, final LocaleResolver resolver) {
			return new BasicAttributeEvaluatorFactory(createELEvaluator(context));
		}

		private AttributeEvaluator createELEvaluator(final ApplicationContext context) {
			if (tilesElPresent) {
				final AttributeEvaluator evaluator = new TilesElActivator().createEvaluator();
				if (evaluator != null) {
					return evaluator;
				}
			}
	...
	private class SpringCompleteAutoloadTilesContainerFactory extends CompleteAutoloadTilesContainerFactory {
		...
		@Override
		public TilesContainer createContainer(final ApplicationContext applicationContext) {
			final CachingTilesContainer cachingContainer = (CachingTilesContainer) super.createContainer(applicationContext);
			final BasicTilesContainer tilesContainer = (BasicTilesContainer) cachingContainer.getWrappedContainer();
			final BeanWrapper bw = PropertyAccessorFactory.forBeanPropertyAccess(tilesContainer.getDefinitionsFactory());
			if (bw.isWritableProperty("localeResolver")) {
				bw.setPropertyValue("localeResolver", new HackedSpringLocaleResolver());
			}
